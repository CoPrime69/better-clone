...
const NAV_ITEMS = ["Buy", "Refinance", "HELOC", "Rates", "Better+"];
...
export default function Header() {
  ...
  const isSpecialPage =
    pathname === "/about-us" || pathname === "/mortgage-calculator";

  useEffect(() => {
    // Only apply scroll effects on regular pages
    if (!isSpecialPage) {
      const handleScroll = () => {
        setIsScrolled(window.scrollY > window.innerHeight);
        setShowGetStarted(window.scrollY > window.innerHeight * 1.5);
      };

      window.addEventListener("scroll", handleScroll);
      return () => window.removeEventListener("scroll", handleScroll);
    } else {
      // Clean up any existing listeners when on special pages
      return () => {};
    }
  }, [isSpecialPage]);

  // Determine if light theme should be used
  const isLight = isSpecialPage || isScrolled;

  return (
    <header
      className={`sticky top-0 z-20 transition-all ease-in-out duration-300 ${
        isLight ? "bg-white shadow-sm" : "bg-[#004733]"
      }`}
    >
      <nav className="m-auto flex max-w-screen-2xl justify-between p-5 md:py-5 md:px-10 xl:p-5">
        ...
        <div className="flex flex-row">
          ...
          <ul className="flex items-center">
            <li className="mr-2">
              <Link
                className={`font-normal ${
                  isLight ? "text-[#292B29]" : "text-white"
                }`}
                href="/"
              >
            ...
            {NAV_ITEMS.map((title) => (
              <DesktopNavItem key={title} title={title} isLight={isLight} />
            ))}
          </ul>
        </div>
        ...
        <ul className="flex items-center gap-3 lg:gap-6">
          ...
          {/* Get Started Button */}
          {!isSpecialPage && showGetStarted && (
            <li className="transition-all duration-500 ease-in">
              <Link
        ...
        >
          <svg
            ...
            className={`transition-all ease-in-out group-hover:[&_path]:fill-[#004733] ${
              isLight ? "[&_path]:fill-[#292B29]" : "[&_path]:fill-white"
  ...
  return (
    <>
      ...
      >
        ...
        {NAV_ITEMS.map((title) => (
          <MobileNavItem key={title} title={title} />
        ))}
      ...
      {isOpen && (
        <button
          ...
          className="bg-black fixed inset-0 h-screen w-screen opacity-80 z-1 focus:border transition-translate duration-300 ease-[cubic-bezier(0.42,0,0.58,1)]"
          onClick={onClose}
        ></button>
      )}
    </>
...
function MobileNavItem({ title }: NavItemProps) {
...
function DesktopNavItem({ title, isLight }: NavItemProps) {
  return (
